...
<p>
	<a class="btn btn-warning" href="<?=hlien("location","edit","id",0)?>">Ajouter
...
foreach ($result as $row) {
    ...
    if ($ligne) {
		...
		<td><a class="btn btn-info"
			href="<?=hlien("location","edit","id",$row["loc_id"])?>">Modifier <span
				class="sr-only">la demande n�<?=mhe($row['loc_id'])?></span></td>
		</a>
		</td>
		</a>
		</td>
		<td><a
			onclick="return confirm('êtes-vous sûr de vouloir supprimer location n°<?=mhe($row['loc_id'])?>?')"
			class="btn btn-danger"
			href="<?=hlien("location","del","id",$row["loc_id"])?>">Supprimer <span
				class="sr-only">la demande n�<?=mhe($row['loc_id'])?></span></a></td>
	</tr>
		<?php } ?>
	</table>